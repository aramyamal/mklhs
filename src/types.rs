...
/// Aggregated evaluated signature $\tilde\sigma = (\gamma, \{\mu_{\mathsf{id}}\})$.
#[derive(Clone, Debug)]
...
/// Evaluated signature for mkqhs-br (baseline, O(tR) size).
///
...
/// Evaluated signature for mkqhs_cbr (compressed, O(t + R) size).
///
...
/// Evaluated signature for mkqhs-br-msq (O(tR) size).
...
/// Evaluated signature for mkqhs-cbr-msq (O(t + R) size).
